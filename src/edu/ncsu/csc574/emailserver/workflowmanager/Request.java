...
public class Request implements IRequest {
	...
	public Request(String command, HashMap<String, String> headerLines,
			byte[] body) {
	...
	public static IRequest parse(BufferedReader br) throws Exception {
		...
		for (int i = 0; i < nrHeaderLines; i++) {
			...
			if (headerLine == null) {
				...
				throw new Exception("Line no:" + (i + 3)
						+ " is Empty in original file");
			...
			if (linePairs.length != 2) {
				...
				throw new Exception("Invalid request: header line containing: "
						+ linePairs);
	...
	public String getRawRequest() {
		...
		br.append(headerLines.size());// nrHeaders + cmd + headers size
		...
		while (itr.hasNext()) {
			...
			br.append(createHeaderLine(headerLine.getKey(),
					headerLine.getValue()));
